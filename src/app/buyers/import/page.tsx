...
export default function ImportPage() {
  ...
  const [parsedRows, setParsedRows] = useState<any[]>([]);   // stores valid parsed csv rows
  const [errors, setErrors] = useState<ValidationError[]>([]);   // stores validation errors
  ...
  function normalizeBhk(rawBhk: unknown): string | undefined {   // normalize bhk
  ...
  const handleFile = (e: React.ChangeEvent<HTMLInputElement>) => {   // handles csv file input, parse and validates rows
    ...
    Papa.parse(file, {
      ...
      complete(results) {
        ...
        rows.slice(0, 200).forEach((row, index) => {   // validates 200 rows
          let normalizedBhk = normalizeBhk(row.bhk);

          if (   // conditional validation for bhk
            row.propertyType !== "Apartment" &&
            row.propertyType !== "Villa"
          ) {
          ...
          const result = buyerSchema.safeParse(row);   // validate rows
          if (!result.success) {
            errList.push({
              row: index + 2,
  ...
  const handleImport = async () => {   // sends validated rows to backend
    setIsLoading(true);
    ...
    try {
      ...
      setParsedRows([]);   // clears after import
    } catch (err: any) {
