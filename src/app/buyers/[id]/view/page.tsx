...
type Buyer = {   // structure for buyer info
  id: string;
  fullName: string;
  phone: string;
  email?: string;
...
type BuyerHistoryRecord = {   // stores a single change record + before -> after snapshot
  id: string;
  changedBy: string;
  changedAt: string;
  diff: {
...
export default function BuyerViewPage() {
  ...
  const [buyer, setBuyer] = useState<Buyer | null>(null);   // store buyer details
  const [history, setHistory] = useState<BuyerHistoryRecord[]>([]);   // recent change history
  const [loading, setLoading] = useState(true);   // loading state
  const [error, setError] = useState<string | null>(null);   // stores errors

  useEffect(() => {   // fetch buyer details and recent history
    if (!id) return;
    ...
    async function fetchData() {
      ...
      try {
        ...
        const historyRes = await fetch(
          `/api/buyer_history?buyerId=${id}&limit=5&sort=desc`
        );
  ...
  function renderDiff(before: any, after: any) {   // renders diff between old and new fields
    const fields = Object.keys({ ...before, ...after });   // get keys from before and after objects
      ...
      <div className="max-w-4xl mx-auto p-6 space-y-8 bg-white rounded-md shadow-md">
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 gap-6 border-b pb-6">
          <div className="space-y-2">
            <p>
              <span className="font-bold text-blue-400/80">Phone:</span>{" "}
              {buyer.phone}
            </p>
            <p>
              <span className="font-bold text-blue-400/80">Email:</span>{" "}
              {buyer.email ?? "-"}
            </p>
          </div>
          <div className="space-y-2">
        ...
        <div className="pt-4 border-t">
          <Button
            onClick={() => router.back()}
            variant="outline"
            className="cursor-pointer text-blue-500 border-blue-500 hover:text-blue-500"
          >
            Back
          </Button>
        </div>
