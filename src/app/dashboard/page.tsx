...
export default function Dashboard() {
  ...
  const [buyers, setBuyers] = useState([]);   // stores array of buyer leads
  const [loading, setLoading] = useState(true);   // initial fetch loading state
  const [refreshing, setRefreshing] = useState(false);   // refreshing state for manual refresh
  const [userDetails, setUserDetails] = useState<{   // stores logged-in user detials or null if not loaded
    name: string;
    email: string;
    phone: string;
    id?: string;
  ...
  async function fetchUserAndBuyers(isInitial = false) {   // fetches user details and leads
    ...
    try {
      ...
      setUserDetails({   // update user details with fetched data
      ...
      const res = await fetch("/api/my-leads", {   // fetch buyer leads
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      });
  ...
  const handleRefresh = () => {   // handles manual refresh
    fetchUserAndBuyers(false);
  };

  if (loading) {   // initial page load
    return (
        ...
        <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight">
              Welcome,{" "}
              <span className="text-blue-400">{userDetails?.name}</span> 👋
            </h1>
          ...
          <Button
            className="text-blue-500 bg-transparent border-2  bold cursor-pointer hover:text-white border-blue-500 hover:bg-blue-400"
        ...
        <section>
          ...
          {buyers.length === 0 ? (
            <p className="text-gray-500">
              You currently have no buyers. Start adding some leads!
            </p>
          ) : (
